...
| `path_switch_safety_clearance_loss` | 0.15 m | 새 후보가 이만큼 더 벽에 가까우면 기존 path 유지 후보로 본다 |
| `path_switch_safety_max_length_sacrifice` | 1.60 m | 더 안전한 기존 path를 유지하기 위해 감수할 수 있는 후보 대비 최대 길이 손해 |
| `path_switch_safety_max_start_offset` | 0.95 m | 기존 path를 유지할 수 있는 현재 pose 기준 최대 path offset |
| `path_switch_safety_statuses` | `NORMAL, ALIGN, REJOIN` | 낮은 clearance 후보 보류를 허용하는 DWA 상태. `RECOVERY/EMERGENCY` 계열은 제외 |
